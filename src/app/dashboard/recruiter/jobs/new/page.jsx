...
export default function PostJobPage() {
  ...
  const currentLimit = MOCK_COMPANY.limits[MOCK_COMPANY.plan];
  const isLimitReached = MOCK_COMPANY.activeJobsCount >= currentLimit;
  const canPost = MOCK_COMPANY.isApproved && !isLimitReached;
        ...
        <div className={`mb-6 p-4 rounded-lg flex items-center justify-between text-sm border ${isLimitReached ? "bg-red-950/30 border-red-800 text-red-200" : "bg-zinc-900/50 border-zinc-800 text-zinc-300"
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-0.5">
              Usage: {MOCK_COMPANY.activeJobsCount} / {currentLimit} active slots used.
            </p>
          </div>
          <div>
            {!MOCK_COMPANY.isApproved && (
            ...
            {MOCK_COMPANY.isApproved && isLimitReached && (
              <span className="bg-red-500/20 text-red-400 px-2.5 py-1 rounded text-xs border border-red-500/30">Limit Exceeded</span>
            )}
            {MOCK_COMPANY.isApproved && !isLimitReached && (
